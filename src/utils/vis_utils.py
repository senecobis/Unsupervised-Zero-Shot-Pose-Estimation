...
def draw_reprojection_pair(data, val_results, visual_color_type="conf"):
    ...
    if visual_color_type == "conf":
        if mkpts3d_reprojed.shape[0] != 0:
            ...
            mconf_normalized = (mconf - mconf_min) / (mconf_max - mconf_min + 1e-4)
...
def vis_reproj(
    ...
    assert osp.isfile(
        image_full_path
    ), 'Please parse full image from "Frames.m4v" first.'
...
def save_demo_image(
    pose_pred, K, image_path, box3d_path, draw_box=True, save_path=None
):
    """
...
def dump_wis3d(
    ...
    wis3d.add_keypoint_correspondences(
        image0,
        image1,
        kpts2d,
        kpts2d_reproj,
        ...
        booleans={"inliers": inliers_bool.tolist()},
        meta={
